    ...
    WITH_UNICODE_STRING(env, path, ps) {
        ...
        if (pathlen != 0) {
                 ...
                 ps[1] == L':' && ps[2] == L'\\'))     //absolute
            {
                ...
                // 如果是相对路径，需要验证它的绝对形式是否大于max_path，如果是，需要(1)将其转换为绝对路径和(2)前缀。
                // 这显然是所有相对路径的负担（“驱动器和目录”相对路径的当前 dir/ len被缓存，因此我们只计算一次，但对于
                // “相对驱动器的路径，我们每次都调用_wgetdcwd（）和wcslen（）） ，但如果要支持max_path以外的相对路径，就必须采取措施。
                // 在调用_wfullpath()之前，没有办法预测绝对路径的长度(以便分配足够的内存块)，我们必须先得到“current”dir的长度。
